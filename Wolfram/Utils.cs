...
namespace Wolfram.NETLink
{
	public abstract class Utils
	{
		...
		static Utils()
		{
			junkChars = new char[2] { ' ', '\0' };
			isWin = Environment.OSVersion.ToString().IndexOf("Windows") != -1;
		...
		public static decimal DecimalFromString(string s)
		{
			...
			{
				s = s.Substring(0, (num3 != -1) ? num3 : num4) + "e" + (char.IsDigit(s[num4 + 2]) ? "+" : "") + s.Substring(num4 + 2);
			}
			else if (num3 != -1)
			{
				s = s.Substring(0, num3);
		...
		{
			ml.PutFunction("EvaluatePacket", 1);
			ml.PutFunction("EvaluateToTypeset", 2 + ((!useStdForm) ? 1 : 0) + ((pageWidth > 0) ? 1 : 0));
			ml.Put(obj);
			if (!useStdForm)
		...
		{
			ml.PutFunction("EvaluatePacket", 1);
			ml.PutFunction("EvaluateToImage", 2 + (useFE ? 1 : 0) + ((dpi > 0) ? 1 : 0) + ((width > 0 || height > 0) ? 1 : 0));
			ml.Put(obj);
			if (useFE)
				ml.Put(b: true);
			ml.Put(graphicsFmt);
		...
		internal static object readArgAs(IKernelLink ml, int argType, Type t)
		{
			...
			case 1:
				if (t.IsEnum)
				{
					object obj = readArgAs(ml, argType, Enum.GetUnderlyingType(t));
			...
			case TypeCode.Object:
				...
				default:
					...
					if (t.IsPointer && argType == 8)
						return readArgAs(ml, 8, TypeLoader.GetType(t.GetElementType().FullName + "[]", throwOnError: true));
					throw new ArgumentException();
				}
			case TypeCode.Byte:
				if (argType != 1)
			...
			if (t == typeof(Array))
			{
				...
				try
				{
					...
					case 1:
					{
						...
						ExpressionType exprType = ml.GetNextExpressionType();
						ml.SeekMark(mark);
						Type leafType = netTypeFromExpressionType(exprType, ml);
						if (leafType == typeof(Expr))
						{
							if (exprType == ExpressionType.Complex)
					...
					case 2:
					{
						...
						bool flag = false;
						bool flag2 = false;
						ExpressionType exprType = ExpressionType.Integer;
						int num3 = ml.CheckFunction("List");
						if (num3 > 0)
						{
							exprType = ml.GetNextExpressionType();
							flag2 = true;
						}
						...
						if (flag)
						{
							...
							{
								if (ml.GetNextExpressionType() == ExpressionType.Function)
								{
									array.SetValue(ml.GetArray(leafType, 1), m);
									continue;
								}
								if (ml.GetSymbol() != "Null")
									throw new MathLinkException(1003);
					...
					default:
					{
						...
						Type leafType = netTypeFromExpressionType(ml.GetNextExpressionType(), ml);
						ml.SeekMark(mark);
