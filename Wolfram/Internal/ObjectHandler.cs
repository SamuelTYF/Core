	...
	internal class ObjectHandler
	{
		private class TypeRecord
		{
			...
			internal TypeRecord(Type t)
			{
				...
				if (t.IsInterface)
				{
					MethodInfo[] array = typeof(object).GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public);
					MethodInfo[] array2 = new MethodInfo[mis.Length + array.Length];
					mis.CopyTo(array2, 0);
					array.CopyTo(array2, mis.Length);
					mis = array2;
				}
		...
		private class InstanceCollection
		{
			internal class InstanceCollectionEnumerator
			{
				...
				public object Current
				{
					get
					{
						ulong key = ((ulong)(uint)((DictionaryEntry)topLevelEnumerator.Current).Key << 24) | (uint)((DictionaryEntry)withinBucketEnumerator.Current).Key;
			...
			internal object get(ulong key)
			{
				if (key == 0)
					return null;
				return ((Bucket)table[(uint)(key >> 24)]).get((uint)(key & 0xFFFFFF));
			}
			...
			internal ulong put(object obj, string alias)
			{
				...
				return ((ulong)hashCode << 24) | bucket.put(obj, alias);
			}

			internal bool addAlias(ulong key, string alias)
			{
				return ((Bucket)table[(uint)(key >> 24)]).addAlias((uint)(key & 0xFFFFFF), alias);
			}
			...
			internal void remove(ulong key)
			{
				...
				if (bucket != null)
				{
					if (bucket.size() == 1)
						...
						bucket.remove((uint)(key & 0xFFFFFF));
				}
			}
		...
		private class Bucket
		{
			...
			{
				return ((BucketRec)table[withinBucketKey]).addAlias(alias);
			}
			...
			{
				foreach (BucketRec value in table.Values)
				{
					object obj2 = value.obj;
					if (object.ReferenceEquals(obj2, obj) || (value.compareValuesOnly && obj.Equals(obj2)))
						return value.key;
				}
				return 0u;
			}
		...
		internal void releaseInstance(string[] objectSyms)
		{
			...
			{
				ulong key = keyFromMmaSymbol(objectSyms[i]);
		...
		internal unsafe void putReference(IKernelLink ml, object obj, Type upCastCls)
		{
			...
			if (flag4 || flag5)
			{
				...
				ml.PutFunction("NETLink`Package`createInstanceDefs", (flag2 || flag3) ? 7 : 5);
				ml.Put(obj2);
				ml.Put(assemblyQualifiedName);
				ml.PutSymbol(s);
				ml.Put(flag6);
		...
		internal void peekObjects(IKernelLink ml)
		{
			...
			try
			{
				...
				{
					ml.PutSymbol((string)enumerator.Current);
				}
			}
			finally
			{
		...
		private object callMethod(IKernelLink ml, int callType, TypeRecord t, object obj, string memberName, int[] argTypes, out OutParamRecord[] outParams)
		{
			...
			if (err != 0)
			{
				...
				{
				case 2:
					text = ((!flag) ? ((!flag2) ? ((!flag3) ? "methargc" : "parampropargc") : "indxrargc") : "ctorargc");
					break;
				case 3:
					text = ((!flag) ? ((!flag2) ? ((!flag3) ? "methodargs" : "parampropargs") : "indxrargs") : "ctorargs");
					break;
				default:
					text = ((!flag) ? ((!flag2) ? "nomethod" : ((callType == 3) ? "indxrnoget" : "indxrnoset")) : "noctor");
					break;
				}
				throw new CallNETException(text, t.fullName, memberName);
			}
		...
		private unsafe MethodBase pickBestMethod(IKernelLink ml, IList matchingMethods, int[] argTypes, out object[] args)
		{
			...
			for (int j = 0; j < argTypes.Length; j++)
			{
				...
				bool flag4 = true;
				Type type = null;
				foreach (MethodRec methodRec5 in methodRecs)
				{
					ParameterInfo parameterInfo2 = methodRec5.pia[j];
				...
				if (flag)
				{
					...
					{
						ParameterInfo parameterInfo3 = ((MethodRec)methodRecs[num2]).pia[j];
				...
				{
					bool flag6 = false;
					foreach (MethodRec methodRec6 in methodRecs)
					{
						ParameterInfo pi2 = methodRec6.pia[j];
				...
				{
					bool flag5 = false;
					foreach (MethodRec methodRec7 in methodRecs)
					{
						ParameterInfo pi = methodRec7.pia[j];
				...
				case 7:
				{
					...
					Type type14 = obj.GetType();
					for (int num7 = methodRecs.Count - 1; num7 >= 0; num7--)
					{
						ParameterInfo pi18 = ((MethodRec)methodRecs[num7]).pia[j];
						if (!Utils.IsOutOnlyParam(pi18) && !isWideningConversion(type14, pi18))
				...
				case 6:
				{
					...
					{
						if (!((MethodRec)methodRecs[num6]).pia[j].IsOptional)
				...
				case 8:
				{
					bool flag59 = false;
					Type type15 = null;
					foreach (MethodRec methodRec8 in methodRecs)
					{
						ParameterInfo pi19 = methodRec8.pia[j];
						type15 = GetParamType(pi19);
						if (!Utils.IsOutOnlyParam(pi19) && type15 == typeof(Expr[]))
						{
							args[j] = Utils.readArgAs(ml, num, typeof(Expr[]));
							flag59 = true;
							break;
					...
					case ExpressionType.Object:
						foreach (MethodRec methodRec9 in methodRecs)
						{
							ParameterInfo pi20 = methodRec9.pia[j];
							type15 = GetParamType(pi20);
							if (!Utils.IsOutOnlyParam(pi20))
							{
								if (!type15.IsArray)
								{
									args[j] = Utils.readArgAs(ml, num, type15);
									break;
								}
								Type elementType6 = type15.GetElementType();
					...
					case ExpressionType.Real:
					{
						bool flag72 = false;
						bool flag73 = false;
						foreach (MethodRec methodRec10 in methodRecs)
						{
							ParameterInfo pi22 = methodRec10.pia[j];
							type15 = GetParamType(pi22);
							if (!Utils.IsOutOnlyParam(pi22))
							{
								if (type15 == typeof(double[]) || type15 == typeof(double*))
									flag72 = true;
								else if (type15 == typeof(decimal[]) || type15 == typeof(decimal*))
								{
									flag73 = true;
								}
							}
					...
					case ExpressionType.Integer:
					{
						...
						bool flag70 = false;
						bool flag71 = false;
						foreach (MethodRec methodRec11 in methodRecs)
						{
							ParameterInfo pi21 = methodRec11.pia[j];
							type15 = GetParamType(pi21);
							if (!Utils.IsOutOnlyParam(pi21) && !type15.IsEnum)
							{
								switch (Type.GetTypeCode(type15.IsArray ? type15.GetElementType() : null))
								{
								case TypeCode.Int32:
									flag65 = true;
									break;
						...
						Type type16 = null;
						type16 = (flag69 ? typeof(decimal[]) : (flag67 ? typeof(long[]) : (flag68 ? typeof(ulong[]) : (flag65 ? typeof(int[]) : (flag66 ? typeof(uint[]) : (flag63 ? typeof(short[]) : (flag64 ? typeof(ushort[]) : (flag62 ? typeof(char[]) : (flag60 ? typeof(byte[]) : (flag61 ? typeof(sbyte[]) : (flag71 ? typeof(double[]) : ((!flag70) ? typeof(int[]) : typeof(float[])))))))))))));
						args[j] = Utils.readArgAs(ml, num, type16);
						cullOutIncompatibleArrayMethods(methodRecs, j, type16.GetElementType());
						break;
					}
				...
				case 9:
				{
					...
					bool flag8 = false;
					Type type2 = null;
					foreach (MethodRec methodRec12 in methodRecs)
					{
						ParameterInfo pi3 = methodRec12.pia[j];
					...
					bool flag9 = false;
					bool flag10 = false;
					foreach (MethodRec methodRec13 in methodRecs)
					{
						ParameterInfo pi4 = methodRec13.pia[j];
						type2 = GetParamType(methodRec13.pia[j]);
					...
					case ExpressionType.String:
						...
						break;
					case ExpressionType.Object:
						foreach (MethodRec methodRec14 in methodRecs)
						{
							ParameterInfo pi5 = methodRec14.pia[j];
					...
					case ExpressionType.Real:
					{
						bool flag23 = false;
						bool flag24 = false;
						foreach (MethodRec methodRec15 in methodRecs)
						{
							ParameterInfo pi7 = methodRec15.pia[j];
					...
					case ExpressionType.Integer:
					{
						...
						bool flag21 = false;
						bool flag22 = false;
						foreach (MethodRec methodRec16 in methodRecs)
						{
							ParameterInfo pi6 = methodRec16.pia[j];
				...
				case 10:
				{
					...
					bool flag42 = false;
					Type type10 = null;
					foreach (MethodRec methodRec17 in methodRecs)
					{
						ParameterInfo pi13 = methodRec17.pia[j];
					...
					bool flag43 = false;
					bool flag44 = false;
					foreach (MethodRec methodRec18 in methodRecs)
					{
						ParameterInfo pi14 = methodRec18.pia[j];
					...
					case ExpressionType.String:
						...
						break;
					case ExpressionType.Object:
						foreach (MethodRec methodRec19 in methodRecs)
						{
							ParameterInfo pi15 = methodRec19.pia[j];
							if (!Utils.IsOutOnlyParam(pi15))
							{
								...
								int arrayRank4 = type10.GetArrayRank();
					...
					case ExpressionType.Real:
					{
						bool flag57 = false;
						bool flag58 = false;
						foreach (MethodRec methodRec20 in methodRecs)
						{
							ParameterInfo pi17 = methodRec20.pia[j];
					...
					case ExpressionType.Integer:
					{
						...
						bool flag55 = false;
						bool flag56 = false;
						foreach (MethodRec methodRec21 in methodRecs)
						{
							ParameterInfo pi16 = methodRec21.pia[j];
						...
						Type type12 = null;
						type12 = (flag54 ? (flag43 ? typeof(decimal[,,]) : typeof(decimal[][])) : (flag52 ? (flag43 ? typeof(long[,,]) : typeof(long[][])) : (flag53 ? (flag43 ? typeof(ulong[,,]) : typeof(ulong[][])) : (flag50 ? (flag43 ? typeof(int[,,]) : typeof(int[][])) : (flag51 ? (flag43 ? typeof(uint[,,]) : typeof(uint[][])) : (flag48 ? (flag43 ? typeof(short[,,]) : typeof(short[][])) : (flag49 ? (flag43 ? typeof(ushort[,,]) : typeof(ushort[][])) : (flag47 ? (flag43 ? typeof(char[,,]) : typeof(char[][])) : (flag45 ? (flag43 ? typeof(byte[,,]) : typeof(byte[][])) : (flag46 ? (flag43 ? typeof(sbyte[,,]) : typeof(sbyte[][])) : (flag56 ? (flag43 ? typeof(double[,,]) : typeof(double[][])) : ((!flag55) ? (flag43 ? typeof(int[,,]) : typeof(int[][])) : (flag43 ? typeof(float[,,]) : typeof(float[][]))))))))))))));
						if (flag43)
						{
							args[j] = Utils.readArgAs(ml, num, type12);
							cullOutIncompatibleArrayMethods(methodRecs, j, type12.GetElementType());
						}
						else if (flag44)
						{
							Type type13 = Array.CreateInstance(type12, 0).GetType();
							args[j] = Utils.readArgAs(ml, num, Array.CreateInstance(type13, 1).GetType());
							cullOutIncompatibleArrayMethods(methodRecs, j, type13);
						}
						else
						{
							args[j] = Utils.readArgAs(ml, num, typeof(Array));
					...
					case ExpressionType.Complex:
						...
						{
							Type type11 = Array.CreateInstance(Array.CreateInstance(ml.ComplexType, 0).GetType(), 1).GetType();
							args[j] = Utils.readArgAs(ml, num, Array.CreateInstance(type11, 1).GetType());
							cullOutIncompatibleArrayMethods(methodRecs, j, type11);
						}
						else
						{
							args[j] = Utils.readArgAs(ml, num, typeof(Array));
				...
				case 11:
				{
					...
					case 2:
					{
						...
						bool flag26 = false;
						Type type6 = null;
						foreach (MethodRec methodRec22 in methodRecs)
						{
							ParameterInfo pi9 = methodRec22.pia[j];
							...
							if (type6.IsArray)
							{
								...
								if (type6.GetArrayRank() == 1 && elementType3.IsArray)
								{
									flag26 = true;
									break;
								}
						...
						case ExpressionType.String:
							...
							break;
						case ExpressionType.Object:
							foreach (MethodRec methodRec23 in methodRecs)
							{
								ParameterInfo pi11 = methodRec23.pia[j];
						...
						case ExpressionType.Real:
						{
							bool flag27 = false;
							bool flag28 = false;
							foreach (MethodRec methodRec24 in methodRecs)
							{
								ParameterInfo pi10 = methodRec24.pia[j];
						...
						case ExpressionType.Integer:
						{
							...
							bool flag39 = false;
							bool flag40 = false;
							foreach (MethodRec methodRec25 in methodRecs)
							{
								ParameterInfo pi12 = methodRec25.pia[j];
					...
					default:
					{
						MethodRec methodRec2 = null;
						MethodRec methodRec3 = null;
						Type complexType = ml.ComplexType;
						foreach (MethodRec methodRec26 in methodRecs)
						{
							ParameterInfo pi8 = methodRec26.pia[j];
						...
						if (methodRec2 != null)
							args[j] = Utils.readArgAs(ml, num, GetParamType(methodRec2.pia[j]));
						else if (methodRec3 != null)
						{
							args[j] = Utils.readArgAs(ml, num, GetParamType(methodRec3.pia[j]));
						}
						if (args[j] == canOnlyMatchOutParam)
							Utils.discardNext(ml);
						break;
				...
				case 13:
				{
					...
					{
						if (!Utils.IsOutOnlyParam(((MethodRec)methodRecs[num3]).pia[j]))
			...
			for (int m = 0; m < count && array[m]; m++)
			{
				...
				if (narrowPrimitives(paramInfos: ((MethodRec)methodRecs[m]).pia, args: args, narrowedArgs: array3))
				{
		...
		private static void cullOutIncompatibleArrayMethods(ArrayList methodRecs, int argPos, Type leafTypeToKeep)
		{
			...
			{
				ParameterInfo parameterInfo = ((MethodRec)methodRecs[num]).pia[argPos];
		...
		private ExpressionType getLeafExprType0(IKernelLink ml, out Type leafObjectType)
		{
			...
			if (ml.GetFunction(out var argCount) == "List")
			{
