...
public class UnsigendBigInterger : IComparable<UnsigendBigInterger>
{
    ...
    public UnsigendBigInterger(params int[] n) => Values = n;
	...
	public static void Divide(UnsigendBigInterger a, UnsigendBigInterger b, out UnsigendBigInterger m, out UnsigendBigInterger n)
	{
		...
		UnsigendBigInterger n2 = new(a.Values[array.Length - 1]);
		array[^1] = Divide(n2, b, lists, out n2);
	...
	{
		return obj is UnsigendBigInterger && this == obj as UnsigendBigInterger;
	}
	...
	public static int SqrtSearch(UnsigendBigInterger a, UnsigendBigInterger b, out UnsigendBigInterger r)
	{
		if (a < b)
		{
			r = a;
		...
		int num = 0;
		int num2 = 10000;
		UnsigendBigInterger unsigendBigInterger = b << 1;
		while (num2 - num > 1)
		{
			int num3 = (num + num2) / 2;
			switch (a.CompareTo((unsigendBigInterger + num3) * num3))
			{
			case 0:
				r = 0;
				return num3;
		...
		r = a - (unsigendBigInterger + num) * num;
		return num;
	}
	internal static UnsigendBigInterger Sqrt(UnsigendBigInterger a)
	{
		if (a == 0)
			return 0;
		UnsigendBigInterger unsigendBigInterger = 0;
		int num = 0;
		int num2 = a.Values.Length - 1;
		int num3 = 0;
		if (a.Values.Length % 2 == 0)
		{
			num = a.Values.Length / 2;
			num3 = a.Values[num2--] * 10000 + a.Values[num2--];
		}
		else
		{
			num = (a.Values.Length + 1) / 2;
			num3 = a.Values[num2--];
		}
		int num4 = (int)Math.Sqrt(num3);
		unsigendBigInterger = (unsigendBigInterger << 1) + num4;
		int num5 = num3 - num4 * num4;
		UnsigendBigInterger r = ((num5 != 0) ? new UnsigendBigInterger(num5) : ((UnsigendBigInterger)0));
		for (int i = 1; i < num; i++)
		{
			r = ((r << 1) + a.Values[num2--] << 1) + a.Values[num2--];
			unsigendBigInterger = (unsigendBigInterger << 1) + SqrtSearch(r, unsigendBigInterger * 2, out r);
		}
