...
namespace Wolfram.NETLink.Internal.COM
{
	internal class COMDispatchHandler
	{
		...
		{
			outParams = null;
			if (!COMUtilities.GetMemberInfo(obj, ref memberName, callType, argTypes.Length == 0, out var paramTypes, out var paramFlags, out var endsWithParamArray))
				throw new CallNETException((callType != 1 && callType != 2) ? "nocommeth" : "nocomprop", "System.__ComObject", memberName);
			...
			case 5:
				if (flag2 && array.Length > 0)
				{
					...
					result = type.InvokeMember(modifiers: new ParameterModifier[1] { parameterModifier }, name: memberName, invokeAttr: BindingFlags.InvokeMethod | BindingFlags.GetProperty, binder: null, target: obj, args: array, culture: null, namedParameters: null);
				}
				else
