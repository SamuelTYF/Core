...
namespace Wolfram.NETLink.Internal
{
	internal class CallPacketHandler
	{
		...
		internal void handleCallPacket(KernelLinkImpl ml)
		{
			...
			catch (Exception lastExceptionDuringCallPacketHandling)
			{
				ml.LastExceptionDuringCallPacketHandling = lastExceptionDuringCallPacketHandling;
			}
			finally
			{
				StdLink.remove();
		...
		private void setComplex(KernelLinkImpl ml)
		{
			...
			try
			{
				string @string = ml.GetString();
				ml.NewPacket();
				ml.ComplexType = TypeLoader.GetType(@string, throwOnError: true);
			}
		...
		private void instanceOf(KernelLinkImpl ml)
		{
			...
			try
			{
				...
				string @string = ml.GetString();
				ml.NewPacket();
				b = TypeLoader.GetType(@string, throwOnError: true).IsInstanceOfType(@object);
			}
			catch (Exception e)
			{
				handleCleanException(ml, e);
		...
		private void reflectType(KernelLinkImpl ml)
		{
			...
			try
			{
				...
				loopbackLink.Put((type.IsGenericType && text.IndexOf(type.Name) > 0) ? text.Substring(text.IndexOf(type.Name)) : type.Name);
				...
				{
					loopbackLink.Put(ObjectHandler.fullTypeNameForMathematica(array[i]));
				}
				...
				{
					putParameterInfo(loopbackLink, array2[j].GetParameters());
				}
		...
		private void delegateTypeName(KernelLinkImpl ml)
		{
			...
			try
			{
				...
				string string2 = ml.GetString();
				ml.NewPacket();
				obj = EventHelper.getDelegateTypeName(@object, @string, string2);
			}
			catch (Exception e)
			{
				handleCleanException(ml, e);
		...
		private void connectToFEServer(KernelLinkImpl ml)
		{
			...
			try
			{
				string @string = ml.GetString();
				ml.NewPacket();
				FEServerLink = MathLinkFactory.CreateMathLink("-linkmode connect -linkname " + @string);
