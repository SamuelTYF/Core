	...
	internal class NativeLink : MathLinkImpl
	{
		...
		public unsafe override string GetString()
		{
			...
			if (api.extMLGetUnicodeString(link, out var strAddress, out var len) == 0)
				throw new MathLinkException(api.extMLError(link), api.extMLErrorMessage(link));
			string result = ((len != 0) ? new string((char*)strAddress.ToPointer(), 0, len) : string.Empty);
		...
		private bool yielderCallbackFunction(IntPtr a, IntPtr b)
		{
			...
			if (Yield != null)
			{
				...
				if (invocationList.Length > 0)
				{
					...
					{
						flag = (bool)array[i].DynamicInvoke(null);
						if (flag)
							break;
					}
				}
				...
				if (File.Exists(path))
				{
					...
					foreach (DictionaryEntry item in sortedList)
					{
						...
						if (File.Exists(value))
							return "-linkmode launch -linkname \"" + value.Replace("\\", "/").Replace("Mathematica.exe", "MathKernel.exe") + "\"";
					}
				}
				string name = (string)Registry.ClassesRoot.OpenSubKey(".nb").GetValue("");
				value = ((string)Registry.ClassesRoot.OpenSubKey(name).OpenSubKey("DefaultIcon").GetValue("")).Split(',')[0];
				if (value.StartsWith("\""))
