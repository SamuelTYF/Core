...
namespace Net.Html
{
	public static class Manager
	{
		...
		{
			using NodeBuilder nodeBuilder = new NodeBuilder(Code.Work_Bytes(Encoding.UTF8.GetBytes(s)));
		...
		{
			using NodeBuilder nodeBuilder = new NodeBuilder(Code.Work_Normal(Url, Referer, Host, UserAgent, Cookie, Encoding));
			return nodeBuilder.Search();
		}
		public static HtmlNode CreateFromUrl_StringCookie(string Url, string Referer = null, string Host = null, string UserAgent = null, string Cookie = null, Encoding Encoding = null)
		{
			...
			using NodeBuilder nodeBuilder = new NodeBuilder(Code.Work_Normal(Url, Referer, Host, UserAgent, cookieContainer, Encoding));
			return nodeBuilder.Search();
		}
		public static HtmlNode CreateFromStream(Stream stream)
		{
			using NodeBuilder nodeBuilder = new NodeBuilder(Code.Work_Stream(stream));
			return nodeBuilder.Search();
		}
		public static HtmlNode CreateFromRequest(HttpWebRequest request)
		{
			using NodeBuilder nodeBuilder = new NodeBuilder(Code.Work_Stream(request.GetResponse().GetResponseStream()));
			return nodeBuilder.Search();
		}
	}
}
